    ...
    for col in ["MUNICIPIO", "MUNICIPIO_ORIGEM", "CATEGORIA", "NOME"]:
        if col in df.columns:
            df[col] = df[col].astype(str).apply(
                lambda s: remover_acentos(s).strip().upper()
            )
    return df
...
# ==============================
# 🔧 Evita repetição no mesmo dia e município de origem
# ==============================
...
# ==============================
# 🔧 Peso de frequência semanal (BLINDADO)
# ==============================
...
def processar_distribuicao(arquivo):
    ...
    for (dia, data, municipio, categoria_oper, qtd), _ in operacoes:
        ...
        candidatos["MATCH_COUNT"] = candidatos["CATEGORIA"].apply(
            lambda c: matching_count_fallback(c, categoria_oper)
        )

        candidatos, vanessa_ativa = aplicar_regra_vanessa(
            candidatos, categoria_oper, data
        )
        if vanessa_ativa:
        ...
        candidatos_pesados = aplicar_regra_frequencia(
        ...
        for _, r in pool.iterrows():
            ...
            if r["NOME"] not in nomes_ja:
                ...
                conv_semana_global[(r["NOME"], semana_sel)] = \
                    conv_semana_global.get((r["NOME"], semana_sel), 0) + 1
    ...
    # ==============================
    # 📊 NÃO CONVOCADOS (INTEIRO, SEM CORTES)
    # ==============================
    ...
    df_base = df[
        [
            ...
            "CATEGORIA",
            "PRESIDENTE_DE_BANCA",
            "DIAS_INDISPONIBILIDADE",
            "INICIO_INDISPONIBILIDADE",
            "FIM_INDISPONIBILIDADE"
        ]
    ...
    def esta_totalmente_indisp(row):
        ...
        if pd.notna(inicio) and pd.notna(fim):
            if inicio.date() <= data_inicio_op.date() and \
               fim.date() >= data_fim_op.date():
                return True
        return False
    ...
    convocados_dias = df_conv.groupby(
        ["NOME", "DIA"]
    ).size().reset_index(name="FOI_CONVOCADO")
    ...
    def em_ferias_ou_indisp(row):
        ...
        if pd.notna(row["DIAS_INDISPONIBILIDADE"]) and str(row["DIAS_INDISPONIBILIDADE"]).strip():
            dias = [
                d.strip().upper().replace("Ç", "C").replace("Á", "A")
                for d in str(row["DIAS_INDISPONIBILIDADE"]).split(",")
            ]
        ...
        if pd.notna(row["INICIO_INDISPONIBILIDADE"]) and pd.notna(row["FIM_INDISPONIBILIDADE"]):
            try:
                ...
                if row["INICIO_INDISPONIBILIDADE"].date() <= data_exemplo.date() <= \
                   row["FIM_INDISPONIBILIDADE"].date():
                    return True
            except:
                pass
        return False
    ...
    df_nao_final = df_nao[
        (df_nao["FOI_CONVOCADO"] == 0) &
        (~df_nao.apply(em_ferias_ou_indisp, axis=1))
    ]

    df_nao_final = (
        df_nao_final
        .groupby(["NOME", "CATEGORIA", "PRESIDENTE_DE_BANCA"])["DIA"]
        .apply(lambda x: ", ".join(sorted(x)))
        .reset_index()
        .rename(columns={
            "PRESIDENTE_DE_BANCA": "PRESIDENTE",
            "DIA": "DIAS_NAO_CONVOCADOS"
        })
    )

    df_nao_final = df_nao_final[
        ["NOME", "CATEGORIA", "PRESIDENTE", "DIAS_NAO_CONVOCADOS"]
    ]
...
# ==============================
st.set_page_config(
    page_title="Distribuição 100% Completa",
    page_icon="⚖️",
    layout="centered"
)
...
<style>
.stApp {
    background: linear-gradient(135deg,#003c63,#015e78,#027b91);
...
<p>Sistema completo com regras de prioridade, presidente real e sem erros de peso.</p>
</div>
""", unsafe_allow_html=True)
